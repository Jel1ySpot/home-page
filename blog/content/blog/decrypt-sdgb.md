...
## TLNR 太长不看版
解密 .app 步骤：
1. 创建 `<PCB CODE>.bin` 文件
2. 将 `<PCB CODE>.bin` 文件前 16 字节填充为仓库内对应的 AES 密钥
3. 将后 16 字节填充为 NTFS 头
4. 运行 `fsdecrypt <PCB CODE>.bin 0x200000 <game.app> <out.vhd>`
5. 将 `<PCB CODE>.bin` 的 16 字节 NTFS 头换成 `<out.vhd>` 的前 16 字节
6. 再次运行 `fsdecrypt <PCB CODE>.bin 0x200000 <game.app> <out.vhd>`

vhd 文件内容提取：
```sh
sudo modprobe nbd
sudo qemu-nbd -f raw -c /dev/nbd0 "outer.vhd"
sudo mount /dev/nbd0 /mnt
# /mnt 里面应该有 internal_{N}.vhd
```

脱壳：用 segatools 将游戏运行起来后使用 CheatEngine 浏览内存找到有 UTF-16LE 编码的 `InternalName Assembly-CSharp.dll` 的内存块。提取即可。

> 以下内容为转载，请支持原作喵
> Reference: [[Azalea] MAiMAi 街机音游逆向](https://aza.moe/blog?post%EF%BC%9D2023-11-30-Maimai-Reversing&post=2023-11-30-Maimai-Reversing)
...
![[Pasted image 20251208173925.png]]
qemu-nbd 加载了第一层之后连 fdisk 都是懵的，即使是这样乱来也能挂载上也真是好神奇：
![[Pasted image 20251208173940.png]]
...
![[Pasted image 20251208174002.png]]
...
![[Pasted image 20251208174008.png]]
...
![[Pasted image 20251208174014.png]]
...
![[Pasted image 20251208174024.png]]
...
![[Pasted image 20251208174046.png]]
...
![[Pasted image 20251208174053.png]]
...
![[Pasted image 20251208174100.png]]
...
![[Pasted image 20251208174104.png]]
...
![[Pasted image 20251208174114.png]]
